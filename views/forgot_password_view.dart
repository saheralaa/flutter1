...
import 'package:project1/utilites/dialogs/password_reset_email_send_dialog.dart';
      ...
      listener: (context, state) async {
        if (state is AuthStateForgotPassword) {
          if (state.hasSendEmail) {
            _controller.clear();
            await showDialogPasswordResetEmailSending(context);
          }
          if (state.exception != null) {
            await showErorrDialog(
              context,
              'We could not request. Please make sure that you  are a regestred user, or if not ,regester a user now by going  back  one step ',
            );
          }
        }
      },
